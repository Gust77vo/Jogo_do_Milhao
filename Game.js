...
async function iniciarJogo() {
    ...
    while (jogarNovamente.toUpperCase() === 'S') {
        ...
        do {
            nomeJogador = await askQuestion("NOS INFORME SEU NOME: ");
            if (nomeJogador.trim() === '') {
        ...
        switch (opcao) {
            case '1':
                ...
                for (let j = 0; j < premiacoesPorRodada.length; j++) {
                    ...
                    let parar = await askQuestion("Deseja parar e levar o valor acumulado? [S]/[N]: ");
                    ...
                    let respostaUsuario = (await askQuestion("Qual opção é a correta?: ")).toLowerCase();
        ...
        if (opcao !== '2') { 
            jogarNovamente = await askQuestion("\nDeseja jogar novamente? [S]/[N]: ");
        }
    }
    rl.close(); 
}
